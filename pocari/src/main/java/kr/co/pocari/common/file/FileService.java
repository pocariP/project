/*package kr.co.pocari.common.file;
	...
	*//**
	...
	public BoardFileDTO uploadSingleFile(MultipartFile multipartFile, HttpSession httpSession){
		...
		if(multipartFile != null 
			...
			fileInfo.setOrgNm(orgFileName);
			fileInfo.setNewNm(newFileName);
			fileInfo.setPath(uriPath);
	...
	public void downloadFile(HttpServletResponse response, HttpSession httpSession, BoardFileDTO fileDTO) {
		...
		try {
			...
			File file = new File(httpSession.getServletContext().getRealPath("/")+fileDTO.getPath() + "/" +  fileDTO.getNewNm() );
			...
			String fileName = encodeFileName( fileDTO.getOrgNm() );
